...
(decisions D1–D6, the M01–M17 and P01 milestones, their dependencies and design coverage), its
checks, the conditional follow-ons, and their backlog items. Nothing here was rewritten,
waived or completed by the move. The plan keeps the short disposition table ("What is deferred
from the immediate path"), the active decisions D7 and D8, and the active blockers.
...
See the [glossary](GLOSSARY.md). "This plan" in the moved text means the implementation plan
as it stood before the move; paths are relative to the repository root.
...
## Checks for the deferred experiment
