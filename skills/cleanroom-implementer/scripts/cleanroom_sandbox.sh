...
# audited from the log (connect calls), not blocked.
#
# The log is strace output for the whole process tree; audit it with
# sandbox_audit.py. Exits 2 when bubblewrap or strace is missing or an argument is
# wrong; otherwise exits with the command's status.
set -u
...
exec strace -f -qq -y -e trace=%file,%process,%network -o "$log" \
  bwrap --die-with-parent --clearenv --unshare-pid --unshare-ipc --unshare-uts \
